/**
 * vue-note-backend-config 的调试日志统一入口。
 *
 ...
 * 2. URL 参数   ：?debug=config / ?debug=1 / ?debug=0
 * 3. 环境变量   ：VITE_DEBUG / DEBUG / NODE_DEBUG（Node 与 SSR 场景）
 *
 * 并补齐包本身不支持的「通配符 / 短名」匹配，例如 `note-config:*`、`config`。
...
export const DEBUG_NAMESPACES = ['note-config:config', 'note-config:demo'] as const;
 ...
 * 前缀（'note-config:*'）、短名（'config' 命中 'note-config:config'）、通配（'*config*'）。
 */
...
/**
 * 逗号分隔的批量开关，语义同 debug 包：`'config,-note-config:demo'`。
...
export function describeDebug(): string {
  ...
  return `note-config debug: ${on.length ? on.join(', ') : '（全部关闭）'}`;
}
...
export const logConfig = createLogger('note-config:config');
export const logDemo = createLogger('note-config:demo');
